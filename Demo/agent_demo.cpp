...
#include <Demo/PibrellaBlockchainListener.h>
...
int main(int argc, char *argv[]) {
    ...
    PibrellaBlockchainListener* pibrellaListener = nullptr;
    pibrellaListener = new PibrellaBlockchainListener();
    if(!(runConfigurableAgent(argv[1],pibrellaListener, [pibrellaListener](){
        pibrellaListener->finishTraining();
    })))
    {
        std::cout << "Can't load configuration file'\n";
        delete pibrellaListener;
        printUsage(argv[0]);
    }
    delete pibrellaListener;
}
